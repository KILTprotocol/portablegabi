...
module.exports.exec = (fn, fnArgs) => {
  // wasm file either needs to be in root dir due to fs or resolved with path
  ...
  if (
    ...
    return WebAssembly.instantiate(
      fs.readFileSync(path.resolve(__dirname, wasmFile)),
      go.importObject
    )
      .then(result => {
        ...
        return promiser(
          global[fn],
          ...(Array.isArray(fnArgs) ? fnArgs : [fnArgs])
        )
      })
      .catch(err => {
        console.error(err)
