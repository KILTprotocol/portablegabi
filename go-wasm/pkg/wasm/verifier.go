...
func StartVerificationSession(this js.Value, inputs []js.Value) (interface{}, error) {
	...
	// ignore last value
	attrs := make([]string, len(inputs)-3)
	for i, v := range inputs[2 : len(inputs)-1] {
		attrs[i] = v.String()
	}

	session, msg := credentials.RequestAttributes(SysParams, attrs, inputs[0].Bool(), (uint64) (inputs[1].Int()))
